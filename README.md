...
- 商户产品列表：查看商户已配置的产品、费率及供应商通道信息。其中供应商通道显示格式调整为"渠道名称 | 通道A | 费率"。
  - **批量添加通道功能**：支持批量为商户产品添加供应商通道
    - **交互优化**：移除表格选择列，简化界面操作
    - **默认设置**：批量添加通道弹窗中"是否针对全部商户"选项默认设置为"否"
    - **动态显示**：仅当"是否针对全部商户"选项为"否"时才显示"商户产品"下拉菜单
    - **智能显示**：下拉菜单支持多选与搜索；当选择超过5个时使用折叠标签并通过提示展示选择统计
    - **选项格式**：下拉菜单选项显示为"商户名称 | 产品名称"格式；在未选择行时展示示例选项（如：商户A | 产品A）
### 支付配置模块 (/payment)
- 支付类型管理：支付方式配置、状态管理
