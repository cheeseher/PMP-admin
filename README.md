...
- 机器人管理
  - 指令管理（已完成）
  - 操作记录（已完成：筛选区布局优化，采用流式 Flex 排布）
  - 群组管理（已完成基础）
- 数据清理
- 商户管理
  - 商户列表（已完成：筛选区布局优化，采用流式 Flex 排布，解决横向滚动问题，行间距优化）
  - 商户产品列表（已完成：筛选区布局优化，统一控件宽度，采用流式 Flex 排布，行间距优化）
- 供应商管理
  - 供应商通道列表（已完成：筛选区布局优化，统一控件宽度，采用流式 Flex 排布，行间距优化）
- 数据统计
  - 支付通道统计（已完成：筛选区布局优化，统一控件宽度，采用流式 Flex 排布，行间距优化）
- 代理商管理
  - 代理商列表（已完成：全字段展示、筛选与分页；移除顶部标签页）
  - 代理商提现订单（已创建：基础页面结构；移除顶部标签页）

## 订单管理
- 商户订单管理（已完成：筛选区布局优化）
  - 筛选布局：采用流式 flex 排布（自动换行），行间距 `8px`、列间距 `12px`
  - 控件宽度：下拉统一 `168px`，整体排版更紧凑，避免筛选项独占一行
  - 样式细节：重置 `el-form-item` 默认边距，确保间距由 grid gap 精确控制；移除按钮组额外上边距，确保对齐
  - 商户提现审核（已完成：新增提现审核页面）
